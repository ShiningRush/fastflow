...
func ShareSetOutput(ctx run.ExecuteContext, output map[string]string, bs []byte) error {
...
func (a *ActionA) Run(ctx run.ExecuteContext, params interface{}) error {
	...
	ctx.Trace("run end")
	bs := `{"field_a_1":"value_a_1","field_a_2":"value_a_2"}`
	ShareSetOutput(ctx, input.Output, []byte(bs))
	return nil
}
